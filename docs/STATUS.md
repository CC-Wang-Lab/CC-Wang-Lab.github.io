...
| **Heredocs break in this shell** | `cat <<'EOF'` fails on apostrophes in the content, **and it collapses `\\` to `\`** even though the delimiter is quoted. Use Python or the Write tool for prose files. |
| **Headless Edge returns before it writes the PNG** | `msedge.exe` hands the work to a detached process and exits 0 straight away, so the exit code tells you nothing. The file lands about 3 s later. Measured: waiting on the exit code gave **0 shots out of 16**; polling for the file gave **16 out of 16** from the same command line. `scripts/shoot.py` polls. This, not an orphan process, is what "Edge exits 0 and writes nothing" always was. |
| **Headless Edge cannot go below ~492 CSS px** | Ask for `--window-size=375` and it lays the page out at 492 px and then **crops** the screenshot to 375, which reads as the site overflowing when it does not. Doubling the device scale factor does not help; the clamp is on DIP. 492 is the narrowest honest width, and it still sits inside both the 575.98 and the 767.98 media blocks. A true 375 px check has to be done by hand in devtools. |
| **`scrollbar-gutter: stable` eats 24 px of every screenshot** | The layout viewport is always 24 px narrower than `--window-size`, even with `--hide-scrollbars`. `scripts/shoot.py` adds it back so a requested 1440 really is 1440. |
